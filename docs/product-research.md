# Product research, samples, and launch decisions

Task 5 adds an organization-isolated research workspace. It helps a team decide whether a product idea deserves capital before it becomes an operational catalog item. It does not create inventory, supplier-product relationships, purchase orders, marketplace integrations, or sales behavior.
...
`product_candidates` stores the hypothesis: name, optional brand text and catalog category, primary marketplace URL, status, notes, author, and optional converted product link.

Evidence is recorded in separate append-only histories:

- market snapshots capture marketplace, listing URL, observed price/currency, rating, review count, demand/competition scores, observation time, and notes;
- supplier options can reference an existing `suppliers` row or preserve a temporary lead, with quote cost, currency, MOQ, lead time, quote dates, and preference;
- sample records store order/receipt dates, cost, result, notes, and a flexible JSON checklist;
...
Money and percentages are validated as decimal strings and calculated by PostgreSQL with `NUMERIC`, not JavaScript floating-point arithmetic. Each calculation is saved.
...
`GET /api/research/candidates` is server-paginated and supports search across candidate name, brand, marketplace/listing URL, notes, and supplier/lead names. Filters include status, category, latest decision, risk, supplier presence, and sample presence.
...
| PATCH | `/api/research/samples/:id` | manage | Update sample/checklist result |
...
Research API tests cover decimal calculations, histories, flexible samples, transition enforcement, capital warnings, comparison, member permissions, organization isolation, and atomic/non-duplicate conversion. Client tests cover the candidate form, sample checklist, calculator request/results, comparison selection, and exact Arabic business-term definitions.

Task 6 and later sales/channel automation remain out of scope.
