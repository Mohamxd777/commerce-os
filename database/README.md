...
Migration `004_inventory_ledger_stock_control.sql` adds the immutable inventory movement ledger, trigger-maintained and direct-write-protected balance projections, explicit Goods Receipt posting state, inventory transfers/items, reorder rules, inventory RBAC permissions, controlled movement/bucket/status constraints, idempotency indexes, reconciliation support, and inventory search/filter indexes. Existing receipts remain unposted until explicitly reviewed and posted.

Migration `005_product_research_samples_launch_decisions.sql` adds organization-isolated product candidates, marketplace and supplier evidence, samples, fee assumptions, authoritative unit economics, launch evaluations/settings, evidence metadata, research permissions, and explicit draft catalog conversion support.

Migration `006_product_research_workspace_ux.sql` adds the focused workspace fields: planned selling price, model/GTIN/description, richer manual marketplace observations, supplier quantity tiers and service terms, explicit sample workflow state, and auditable supplier-option linkage for economics. It inserts no product or supplier example data.
